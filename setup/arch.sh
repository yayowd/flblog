...
EOF
sudo tee /etc/nginx/flblog.conf <<<"$config" >/dev/null
sudo sed -i '/# for flblog/,+2d' /etc/nginx/nginx.conf
sudo sed -i '/[[:space:]]\+server {/i # for flblog\ninclude flblog.conf;\n' /etc/nginx/nginx.conf
