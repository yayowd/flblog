...
当你需要一个简约的博客，来：快速写入和阅读文字、简单干净样式、支持无脚本浏览、对个人支持多类别用户、对组织支持多角色用户、支持服务器静态缓存、支持页面保存数据备份、方便安装、迅速响应，那为什么不试试我们的19blog呢。

主要特点：
1、运行在BASH脚本环境中，无需其它运行时；
2、所有数据都在HTML中，无需数据库环境；
3、后端NGINX静态缓存，无额外资源消耗；
